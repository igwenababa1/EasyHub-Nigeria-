...
const ComparisonTable: React.FC<{products: Product[]}> = ({ products }) => {
    ...
    const allSpecKeys = products.reduce((keys, product) => {
        if (product.specs) {
...
export const ComparisonPage: React.FC<ComparisonPageProps> = ({ comparisonList, allProducts, onToggleCompare, onClear, onClose }) => {
  const showComparisonTable = comparisonList.length >= 2;
    ...
    <div className="animate-fade-in-fast min-h-screen">
      <div className="container mx-auto px-6 py-16">
        <div className="flex justify-between items-center mb-12">
            ...
            <div className="flex items-center gap-4">
              {comparisonList.length > 0 && (
                <button onClick={onClear} className="text-gray-400 hover:text-white transition-colors font-semibold">
                    Clear Selection
                </button>
              )}
        ...
        {showComparisonTable ? (
          ...
          <div>
            <div className="text-center mb-12 bg-gray-900/50 p-6 rounded-xl">
              <h2 className="text-2xl font-bold">Select Products to Compare</h2>
              <p className="text-gray-400 mt-2">
                You have selected {comparisonList.length} {comparisonList.length === 1 ? 'product' : 'products'}. Please select at least one more to see a side-by-side comparison.
              </p>
            </div>
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {allProducts.map((product, index) => (
                <ProductCard 
                  key={product.id} 
                  product={product} 
                  onSelect={() => {}} // Not needed here, card is not clickable to detail view
                  index={index}
